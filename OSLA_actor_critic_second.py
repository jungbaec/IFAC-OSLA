...
class Actor(nn.Module):
  ...
  def forward(self, state):
    a = torch.square(self.mu(state)) # a = self.mu(state)
...
class Critic(nn.Module):
  def __init__(self):
    ...
    self.P = nn.Sequential(nn.Linear(2,8), nn.ReLU(), nn.Linear(8,8), nn.ReLU(), nn.Linear(8,1)) 
    self.V = nn.Sequential(nn.Linear(2,8), nn.ReLU(), nn.Linear(8,8), nn.ReLU(), nn.Linear(8,1))
  ...
  def forward(self, x):
    a = x[:,0:1]
    s = x[:,1:]
    P = torch.square(self.P(s)) # P = self.P(s)
    V = self.V(s)
    C = torch.cat((a, P, V), dim = 1) # Concatenate a, P, V as 1 input
    q1 = self.q1(C)
...
def train(episodes, *args):
  ...
  if not args:
    ...
    if len(args) > 1:
      ...
      actor.load_state_dict(torch.load("actor_2_weights.pth")) # trained in 10 bit case
      
  actor_target = copy.deepcopy(actor)
  critic = Critic().to(device)
  critic.load_state_dict(torch.load("critic_2_weights.pth")) # trained in 10 bit case
  critic_target = copy.deepcopy(critic)
  
  memory = ReplayMemory(20000)     # Memory(Capacity) for Experience Replay
  actor_optimizer = optim.Adam(actor.parameters(), lr=3e-4) # Can tune the learning rate smaller one(3e-4, 1e-4)
...
####################### Testing ############################
#torch.save(actor.state_dict(), "actor_2_weights.pth")
#torch.save(critic.state_dict(), "critic_2_weights.pth")
actor = Actor().to(device)
critic = Critic().to(device)
actor.load_state_dict(torch.load("actor_2_weights.pth"))
critic.load_state_dict(torch.load("critic_2_weights.pth"))
...
plt.figure(1)
plt.xlabel('Eb/N0 (linear) for Actor-critic-2') # this is proportional to the T_rem
...
## Evaluate BER
ber_ac_2 = []
ber_for_each_bit_ac_2 = np.zeros([bit_len_test, len(EbN0dB)])
Lthres_ac_2 = torch.zeros([bit_len_test, len(EbN0dB)])
...
for s in EbN0dB:
  ...
  for n in tqdm(range(1,Nsim+1)):
    ...
    if np.min(ber_for_each_bit_ac_2[:, s]) >= 100:
      break

  ber_ac_2.append(err / (n * bit_len_test))
  T_mean /= (n * bit_len_test)
  ber_for_each_bit_ac_2[:, s] /= n
  print(err,n,"\n")
  print("================\n")


print("BER =", ber_ac_2)
print("BER for each bit =", ber_for_each_bit_ac_2)
print("Sum of BER for each bit", np.sum(ber_for_each_bit_ac_2, axis = 0))
    
plt.figure(2)
plt.semilogy(EbN0dB, ber_ac_2, '-o', label="Actor-critic-2, simulation")
...
plt.figure(3)
for i in range(1, bit_len_test + 1):
  plt.semilogy(EbN0dB, ber_for_each_bit_ac_2[i-1], '-o', label = f"BER for bit {i}")
plt.xlabel("Eb / N0 (dB)")
...
plt.figure(4)
for s in EbN0dB:
  plt.semilogy(range(1, bit_len_test + 1), ber_for_each_bit_ac_2[:, s], '-o', label = f"BER for Eb / N0 (dB) {s}")
plt.xlabel("bit")
...
'''
# Save list data
with open("ber_ac_2_bit_len_train_and_test_20_init_param_10_train_only_unobserved.pkl","wb") as f: # _len_test_50
    pickle.dump(ber_ac_2, f)

# Save torch matrix
torch.save(Lthres_ac_2, "Lthres_ac_2_bit_len_train_and_test_20_init_param_10_train_only_unobserved.pt") # _len_test_50
'''
